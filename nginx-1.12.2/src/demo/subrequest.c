...
ngx_int_t demo_subrequest_handler(ngx_http_request_t *r, void *data, ngx_int_t rc)
{
    ...
    if (r->upstream && r->upstream->out_bufs) {
        ngx_chain_t *bufs = r->upstream->out_bufs;
...
{
    r->request_body->bufs = r->request_body->bufs;
    ...
    ngx_str_t location = {13, "/adx_com_fcgi"};
    ngx_http_request_t *sub_request = NULL;
    // demo_conf *conf = ngx_http_get_module_loc_conf(r, demo);
