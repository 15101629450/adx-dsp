...
int adx_link_jump_mapid_load(redisContext *conn, adx_cache_t *root)
{
    ...
    vector<adx_size_t> mapids;
    errcode = adx_redis_command(conn, mapids, "SMEMBERS adx_link_jump_mapid"); // 全部 要要发送的mapid
    if (errcode != E_SUCCESS) return errcode;

    adx_cache_t *adx_link_jump_mapid_cache = adx_cache_add(root, CACHE_STR("adx_link_jump_mapid"), CACHE_NULL()); // 创建 adx_link_jump_mapid
    if (!adx_link_jump_mapid_cache) return E_MALLOC;

    // 遍历 policyids
    for (size_t i = 0; i < mapids.size(); i++) {
	int mapid = mapids[i];
	adx_cache_add(adx_link_jump_mapid_cache, CACHE_NUM(mapid), CACHE_NULL());
    }
...
adx_cache_t *adx_dump_load(adx_dump_redis_conn *conn)
{
    ...
    adx_link_jump_mapid_load(conn->adx_setting, root);
