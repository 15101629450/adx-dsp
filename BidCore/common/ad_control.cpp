...
{
	_lock.Enter();
	Record one = { policyid, mapid, at, regcode, price, GetTimeHourSplit(10) };
	_all.push(one);
	_lock.Leave();
}
...
{
	char key[128]; // key: dsp10minutecounter_(policyid)_(10分钟段)
	sprintf(key, "dsp10minutecounter_%d_%d", one.pid, one.section);
